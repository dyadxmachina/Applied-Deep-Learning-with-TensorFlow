...
from keras.layers import Dropout,BatchNormalization, Permute, Reshape
...
def optimizer(model, targets, global_step):
    ...
    return (train_op,loss, predictions, accuracy)
...
if FLAGS.job_name == 'ps':
    ...
    with tf.device(tf.train.replica_device_setter(
        ...
        train_op,loss, predictions, accuracy = optimizer(model, targets, global_step)
    ...
    #Session
    stop_hooks = [tf.train.StopAtStepHook(last_step = 5000)]
    ...
    with tf.train.MonitoredTrainingSession(master = server.target,
                                          ...
                                          scaffold = scaff,
                                          hooks = stop_hooks) as sess: 
       # tf.reset_default_graph()
        keras.backend.set_session(sess)
        print('Starting training on worker %d'%FLAGS.task_index)
        count = 0
        while not sess.should_stop() and count < 4000:
